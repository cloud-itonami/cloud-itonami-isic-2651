(ns measctrlmfg.operation
  "MeasCtrlOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (MeasCtrlAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Measuring
  Control Equipment Plant Operations Governor (:govern) and the
  ...
  (:require [langgraph.graph :as g]
            ...
            [measctrlmfg.advisor :as advisor]
...
(defn build
  "Compiles a MeasCtrlOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `measctrlmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MeasCtrlAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Measuring Control Equipment Plant Operations Governor -- independent censor.
      (g/add-node :govern
