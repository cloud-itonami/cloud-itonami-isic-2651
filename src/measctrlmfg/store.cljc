(ns measctrlmfg.store
  "SSoT for the measuring/testing/navigating/control-equipment plant-
  operations coordination actor, behind a `Store` protocol so the
  backend is a swap, not a rewrite -- the same seam every
  `cloud-itonami-isic-*` actor in this fleet uses.

  Scope note: like its siblings (`cloud-itonami-isic-2670`'s own
  `opticalmfg.store`, `cloud-itonami-isic-3250`'s own
  `medinstrmfg.store`), this build ships a single `MemStore` backend
  ...
  is self-contained (no external measuring/testing/navigating/control-
  equipment-manufacturing capability library, no jurisdiction-scoped
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             instrument-class/calibration-accuracy/
    ...
    - `equipment`         -- a calibration-bench/assembly-line/test-
    ...
    - `maintenance`       -- a scheduled calibration-bench/assembly-
                             line/test-bench-equipment maintenance-
                             window DRAFT against a piece of equipment
                             (`measctrlmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound measuring/testing/
                             navigating/control-equipment shipment
                             DRAFT (`measctrlmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (measctrlmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (measctrlmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :instrument-class :process-control-instrument
                :lot-number "MCE-2026-001" :calibration-accuracy-ppm 250.0
                :quantity-units 5000.0 :defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 1000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :instrument-class :meter
                :lot-number "MCE-2026-002" :calibration-accuracy-ppm 400.0
                :quantity-units 800.0 :defect-rate-percent 0.3
                :verified? true :registered? true
                :shipped-units 750.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :instrument-class :laboratory-analytical-instrument
                :lot-number "MCE-2026-003" :calibration-accuracy-ppm 620.0
...
(defn- sample-equipment []
  {"calibration-bench-001" {:id "calibration-bench-001" :kind :calibration-bench
                             :verified? true :registered? true
                             :last-maintenance-date "2026-05-01"}
   "test-bench-002" {:id "test-bench-002" :kind :test-bench
                      :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `measctrlmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `measctrlmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  calibration-bench unit (schedulable for maintenance), one
  UNVERIFIED/unregistered test-bench unit (blocks any maintenance
