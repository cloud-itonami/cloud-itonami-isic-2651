(ns measctrlmfg.advisor
  "MeasCtrlAdvisor -- the *contained intelligence node* for the
  measuring/testing/navigating/control-equipment plant-operations
  coordination actor.

  It normalizes production-batch patches (instrument-class/
  calibration-accuracy/quantity/defect-rate), drafts a calibration-
  bench/assembly-line/test-bench-equipment maintenance scheduling
  proposal against a piece of equipment, drafts a safety-concern flag,
  and drafts an outbound measuring/testing/navigating/control-
  ...
  *proposal* (with a rationale + the fields it cited), never a
  committed record and NEVER a real calibration/assembly/test-bench-
  equipment actuation, freight dispatch, or NIST-traceable calibration
  certificate -- see README `What this actor does NOT do`. Every
  output is censored downstream by `measctrlmfg.governor` before
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `measctrlmfg.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct calibration/
  ...
  `measctrlmfg.governor` HARD-holds any request that doesn't, so a
...
(defn- log-production-batch
  ...
  class, calibration-accuracy reading, quantity, or defect-rate, nor
  its verification status. High confidence, low stakes --
...
(defn- schedule-maintenance
  "Draft a calibration-bench/assembly-line/test-bench-equipment
  ...
  registered?) in its rationale, but `measctrlmfg.governor` NEVER
...
(defn- flag-safety-concern
  "Draft a calibration-drift/precision-defect/electrical-safety
  ...
  technicality). See `measctrlmfg.phase`: no phase ever adds this op
  to a phase's `:auto` set; `measctrlmfg.governor` also always
...
(defn- coordinate-shipment
  "Draft an outbound measuring/testing/navigating/control-equipment
  ...
  does NOT invent one, and `measctrlmfg.governor` NEVER trusts it: it
...
(def ^:private system-prompt
  (str "あなたは計測・試験・航法・制御機器製造工場プラント運用コーディネーターの"
       "助言者です。"
       ...
       "校正ベンチ・組立ライン・試験ベンチ等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "NISTトレーサブル校正証明書等を自己発行する提案をしてはいけません。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `measctrlmfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :measctrlmfg-advisor-proposal
   :op         (:op request)
