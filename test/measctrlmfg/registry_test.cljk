(ns measctrlmfg.registry-test
  (:require [clojure.test :refer [deftest is]]
            [measctrlmfg.registry :as r]))
...
(deftest known-instrument-classes-are-valid
  (doseq [c [:process-control-instrument :laboratory-analytical-instrument
...
(deftest fabricated-instrument-class-is-invalid
  (is (not (r/instrument-class-valid? :spectacle-lens)))
  (is (not (r/instrument-class-valid? nil))))

;; ----------------------------- calibration-accuracy-ppm-valid? -----------------------------

(deftest typical-calibration-accuracy-reading-is-valid
  (is (r/calibration-accuracy-ppm-valid? 0.0))
  (is (r/calibration-accuracy-ppm-valid? 250.0))
  (is (r/calibration-accuracy-ppm-valid? 100000.0)))

(deftest negative-calibration-accuracy-reading-is-invalid
  (is (not (r/calibration-accuracy-ppm-valid? -1.0))))

(deftest excessive-calibration-accuracy-reading-is-invalid
  (is (not (r/calibration-accuracy-ppm-valid? 999999.0)))
  (is (not (r/calibration-accuracy-ppm-valid? 100000.01))))

(deftest non-numeric-or-missing-calibration-accuracy-reading-is-invalid
  (is (not (r/calibration-accuracy-ppm-valid? nil)))
  (is (not (r/calibration-accuracy-ppm-valid? "250"))))
...
(deftest maintenance-is-a-draft-not-a-real-actuation
  (let [result (r/register-maintenance "mnt-1" "calibration-bench-001" 0)]
    (is (nil? (get-in result ["certificate" "proof"])))
    (is (= (get-in result ["certificate" "issued_by_registry"]) false))
    (is (= (get-in result ["certificate" "status"]) "draft-unsigned"))))

(deftest maintenance-assigns-maintenance-number
  (let [result (r/register-maintenance "mnt-1" "calibration-bench-001" 7)]
    (is (= (get result "maintenance_number") "MNT-000007"))
    (is (= (get-in result ["record" "maintenance_id"]) "mnt-1"))
    (is (= (get-in result ["record" "equipment_id"]) "calibration-bench-001"))
    (is (= (get-in result ["record" "kind"]) "maintenance-schedule-draft"))
    (is (= (get-in result ["record" "immutable"]) true))))

(deftest maintenance-validation-rules
  (is (thrown? #?(:clj Exception :cljs js/Error) (r/register-maintenance "" "calibration-bench-001" 0)))
  (is (thrown? #?(:clj Exception :cljs js/Error) (r/register-maintenance "mnt-1" "" 0)))
  (is (thrown? #?(:clj Exception :cljs js/Error) (r/register-maintenance "mnt-1" "calibration-bench-001" -1))))
...
(deftest history-is-append-only
  (let [c1 (r/register-maintenance "mnt-1" "calibration-bench-001" 0)
        hist (r/append [] c1)
        c2 (r/register-maintenance "mnt-2" "calibration-bench-001" 1)
