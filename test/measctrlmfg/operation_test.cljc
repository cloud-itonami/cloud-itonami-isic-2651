(ns measctrlmfg.operation-test
  "Smoke tests for the compiled MeasCtrlOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `measctrlmfg.governor-contract-test`; the Store contract in
  `measctrlmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "MeasCtrlOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "calibration-bench-001" :severity :moderate :description "calibration-drift anomaly"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
